...
public class FFT {
  ...
  public static void main(String[] args) throws IOException {
    List<String> lines = Files.readAllLines(Paths.get("1/1.dat"));
    lines.remove(0);
    if (true) {
      // padding 0 to make 2^n
      int l = Integer.highestOneBit(lines.size());
      if (l != lines.size()) {
        for (int i = lines.size(); i < (l << 1); i++) {
          lines.add("0");
        }
      }
    }
    ...
    if (true) {
      Complex[] x = new Complex[lines.size()];
      for (int i = 0; i < x.length; i++) {
        x[i] = new Complex(Double.parseDouble(lines.get(i)), 0);
      }
      for (Complex y : fft(x)) {
        a.add(y.abs() + "");
      }
      ...
      for (int i = 0; i < x.length; i++) {
        x[i] = Double.parseDouble(lines.get(i));
      }
      for (org.apache.commons.math3.complex.Complex y : new FastFourierTransformer(DftNormalization.STANDARD).transform(x, TransformType.FORWARD)) {
        a.add(y.abs() + "");
      }
    }
    Files.write(Paths.get("f.dat"), a);
  }
}
