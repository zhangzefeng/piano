...
/**
 * implementation that uses a YIN algorithm to determine the frequency of the provided waveform data. The YIN algorithm is similar to the Auto-correlation
 * Function used for pitch detection but adds additional steps to better the accuracy of the results. Each step lowers the error rate further. The following
...
public class YINPitchDetector {
  ...
  public YINPitchDetector() {
    this.sampleRate = MainActivity.RECORDING_RATE;
    this.resultBuffer = new float[MainActivity.BUFFER_SIZE / 4 / 2];
  }
  ...
  /**
   * Performs the first and second step of the YIN Algorithm on the provided array buffer values. This is a "combination" of the AutoCorrelation Method and the
   * Difference Function. The AutoCorrelation Method multiplies the array value at the specified index with the array value at the specified index plus the
   * "tau" (greek letter used in the formula). Whereas the Difference Function takes the square of the difference of the two values. This is supposed to provide
   * a more accurate result (from about 10% to about 1.95% error rate). Note that this formula is a riemann sum, meaning the operation specified above is
   * performed and accumulated for every value in the array. The result of this function is stored in a global array, {@link #resultBuffer}, which the
   * subsequent steps of the algorithm should use.
   *
  ...
  /**
   * Performs the third step in the YIN Algorithm on the {@link #resultBuffer}. The result of this function yields an even lower error rate (about 1.69% from
   * 1.95%). The {@link #resultBuffer} is updated when this function is performed.
   */
  ...
  /**
   * Performs step four of the YIN Algorithm on the {@link #resultBuffer}. This is the first step in the algorithm to attempt finding the period of the wave
   * data. When attempting to determine the period of a wave, it's common to search for the high or low peaks or dips of the wave. This will allow you to
   * determine the length of a cycle or its period. However, especially with a natural sound sample, it is possible to have false dips. This makes determining
   * the period more difficult. This function attempts to resolve this issue by introducing a threshold. The result of this function yields an even lower rate
   * (about 0.78% from about 1.69%).
   *
  ...
  /**
   * Further lowers the error rate by using parabolas to smooth the wave between the minimum and maximum points. Especially helps to detect higher frequencies
   * more precisely. The result of this function results in only a small error rate decline from about 0.78% to about 0.77%.
   */
